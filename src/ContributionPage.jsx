...
export default function ContributionPage({ onBackToHome, currentUserData }) {
  ...
  const officialUPI = 'dreamjourneyproject@ptaxis'; 
  const targetThreshold = 7000;          
  ...
  const handlePaymentSubmit = async (e) => {
    ...
    try {
      ...
      if (!isMobile) {
        alert("💻 Desktop/Laptop detected! UTR saved. Please verify via QR code or complete on a mobile device.");
      } else {
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] text-slate-800 font-sans antialiased w-full max-w-full overflow-x-hidden pb-12">
      
      {/* BANNER HEADER AREA */}
      <div className="relative w-full min-h-[280px] bg-gradient-to-br from-[#0c2340] via-[#1a365d] to-[#2a4365] flex flex-col items-center justify-center text-center px-4 sm:px-6 py-8 overflow-hidden">
        <div className="absolute inset-0 opacity-5 bg-[radial-gradient(#fff_1px,transparent_1px)] [background-size:16px_16px]"></div>
        
        {/* 🏠 TOP NAVIGATION BAR */}
        <div className="w-full max-w-7xl flex justify-between items-center z-30 mb-4">
          <button 
            type="button" 
        ...
        <div className="relative z-10 max-w-3xl space-y-3 w-full">
          <span className="inline-flex items-center gap-1.5 bg-white/10 text-white backdrop-blur-sm border border-white/10 px-3 py-1 rounded-full text-[10px] sm:text-[11px] font-bold uppercase tracking-wider max-w-full truncate">
            🛡️ SECURED COMMUNITY WALLET LAYER
          </span>

          <div className="pt-2 pb-2 space-y-2 group select-none w-full">
            <h1 className="text-3xl sm:text-5xl font-black tracking-[0.15em] sm:tracking-[0.3em] font-sans text-center uppercase relative block transition-all duration-500 ease-out cursor-default hover:tracking-[0.25em] break-words">
              ...
              <span className="absolute -bottom-1 left-1/2 -translate-x-1/2 w-24 h-[2px] bg-gradient-to-r from-transparent via-cyan-400 to-transparent shadow-[0_0_8px_#22d3ee]"></span>
            </h1>
            
            <div className="flex items-center justify-center gap-2 pt-2">
              <span className="w-1.5 h-1.5 rounded-full bg-cyan-400 animate-ping duration-1000 shrink-0"></span>
              <p className="text-[10px] sm:text-xs font-black text-slate-400/90 tracking-[0.15em] sm:tracking-[0.2em] uppercase font-mono transition-colors duration-300 group-hover:text-cyan-300">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 -mt-8 relative z-20 grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-6 w-full overflow-hidden">
          ...
          <div className="bg-white border border-slate-100 rounded-3xl p-5 sm:p-8 shadow-xl space-y-4">
            <div className="flex justify-between items-center text-xs font-bold text-slate-400 uppercase tracking-wider">
              <span>🎯 Monthly Contribution Goal</span>
            </div>
            <div className="text-2xl sm:text-3xl font-black text-slate-900 tracking-tight flex flex-wrap items-baseline gap-2">
          ...
          <div className="bg-white border border-slate-100 rounded-3xl p-5 sm:p-8 shadow-xl space-y-6">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4 w-full max-w-3xl mx-auto pt-2">
              {[
                ...
                <div
                  ...
                  className={`flex flex-col items-center justify-between border-2 rounded-2xl sm:rounded-3xl p-3 sm:p-5 min-h-[170px] sm:min-h-[190px] text-center cursor-pointer transition-all duration-200 ${
                ...
                >
                  <div className={`w-4 h-4 sm:w-5 sm:h-5 rounded-full border-2 flex items-center justify-center self-start ${
                    selectedAmount === tier.id ? 'border-sky-500' : 'border-slate-300'
                  }`}>
                    {selectedAmount === tier.id && (
                      <div className="w-2 h-2 sm:w-2.5 sm:h-2.5 rounded-full bg-sky-500"></div>
                    )}
                  </div>
                  ...
                  <div className="my-auto py-1">
                    ...
                    <span className={`inline-block text-[9px] sm:text-[10px] font-extrabold px-2.5 py-0.5 sm:py-1 rounded-full mt-1.5 uppercase tracking-wider ${
                      selectedAmount === tier.id 
                  ...
                  <p className={`text-[10px] sm:text-[11px] font-semibold mt-1 leading-tight px-1 ${
            ...
            >
              <img 
                ...
                style={{ minWidth: '40px' }}
              />
              <span className="whitespace-nowrap text-xs sm:text-base">PAY WITH UPI APP (MOBILE)</span>
            </button>

            {/* Laptop/Desktop QR Code Interface */}
            <div className="border border-slate-100 bg-slate-50/50 rounded-2xl p-5 hidden md:block">
              <div className="flex flex-col sm:flex-row items-center justify-between gap-5">
                <div className="w-28 h-28 bg-white border p-2 rounded-xl flex items-center justify-center shadow-inner overflow-hidden shrink-0">
                  <img 
                ...
                <div className="flex-grow space-y-2.5 w-full text-xs">
                  <div>
                    ...
                    <div className="flex rounded-xl overflow-hidden border bg-white max-w-xs mt-1 shadow-sm">
                      <div className="px-3 py-2 font-mono text-slate-600 flex-grow select-all text-xs truncate">{officialUPI}</div>
                      <button type="button" onClick={handleCopyUPI} className="bg-slate-50 text-sky-500 font-bold px-3 border-l text-[10px] shrink-0">
            ...
            {successAlert && (
              <div className="w-full bg-emerald-50 border border-emerald-200 text-emerald-800 p-4 rounded-xl flex items-center gap-3 transition-all">
            ...
            <form onSubmit={handlePaymentSubmit} className="bg-amber-50/40 border border-amber-200 p-4 rounded-xl space-y-4">
              <div>
                ...
                <input
                  ...
                  maxLength={12}
                  className="w-full bg-white border border-amber-200 mt-1.5 px-3 py-2.5 rounded-xl text-black focus:outline-amber-500 text-sm font-mono"
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full bg-amber-500 hover:bg-amber-600 disabled:bg-gray-400 text-white font-bold py-3.5 rounded-full uppercase tracking-wider transition-all cursor-pointer text-xs sm:text-sm"
              >
            ...
            {!showVerifyStep && (
              <div className="space-y-2 pt-2">
                <button 
                  ...
                  className="w-full bg-slate-900 hover:bg-slate-800 text-white py-4 rounded-xl font-bold text-xs uppercase tracking-wider shadow-md cursor-pointer"
                >
        ...
        <div className="space-y-6 text-xs font-medium w-full">
          <div className="bg-white border border-slate-100 rounded-3xl p-6 shadow-xl space-y-4">
            ...
            <div className="bg-white border border-slate-100 rounded-2xl p-4 shadow-sm mb-4 relative overflow-hidden text-left">
              <div className="flex items-start gap-3">
                ...
                <div className="space-y-1">
                  <h3 className="text-base font-bold text-slate-800">Why Contribute?</h3>
